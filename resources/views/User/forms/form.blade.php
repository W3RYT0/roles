<div class=" py-4 px-2 gap-2 bg-white border border-gray-100 rounded-lg shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-1 gap-x-4 gap-y-1">
        <div class="col-span-2 py-2">
            <div class="relative">
                <input type="text" id="name" name="name" value="{{ old('name') ?? (isset($user) ? $user->name : '') }}" 
                    ...
                    dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-indigo-300 peer"  placeholder=" " />
        ...
        <div x-data="{photoName: null, photoPreview: null}" class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-1 gap-x-4 gap-y-1">
            <div class=" -mt-2 col-span-3 py-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-1 gap-x-4 gap-y-1 ">
        <div class="flex flex-row col-span-2 py-2 mb-2">
            ...
            <input class="flex-auto w-full h-8 text-xs text-gray-500 border border-indigo-100 rounded-none appearance-none rounded-r-md md:text-sm 
            focus:ring-0 focus:border-indigo-300 focus:shadow-md" id="password" name="password" type="password" placeholder="Contraseña" autocomplete="new-password" />
        </div>
        <div class="flex flex-row col-span-2 py-2 mb-2">
            <span class="inline-flex items-center px-3 py-1 text-sm text-gray-500 bg-gray-200 border border-indigo-100 rounded-l-md border-r-1">
                Confirmar
            ...
            <input class="flex-auto w-full h-8 text-xs text-gray-500 border border-indigo-100 rounded-none appearance-none rounded-r-md md:text-sm 
            focus:ring-0 focus:border-indigo-300 focus:shadow-md" id="password_confirmation" name="password_confirmation" type="password" placeholder="Contraseña" 
            autocomplete="off" />
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-1 gap-x-4 gap-y-1 ">
        <div class="flex flex-row col-span-2 py-2 mb-2">
            ...
            <input placeholder="Correo" type="text" name="email" id="email"
                ...
                class="flex-auto  h-10 text-xs text-gray-500 border border-indigo-100 rounded-none appearance-none 
                focus:ring-0 focus:border-indigo-300 focus:shadow-md rounded-r-md md:text-sm" />
        </div>
    </div>
    <hr>
    <input type="hidden" name="id" value=" {{ isset($user) ? $user->id : '' }}">
    <h4 class=" text-md gap-2 px-4 py-2 font-semibold">Asignar rol</h4>
    <div class=" py-4 px-2 gap-2 bg-white border border-gray-100 rounded-lg shadow-md">
        <ul>
