        ...
        <div class="relative w-full mb-4 sm:w-1/2">
            <div class="mx-2">
                ...
                <label for="group" class="absolute text-xs text-gray-500 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 
                    peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-5 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 left-3">Modelo o Grupo al que pertenece el permiso</label>
                <span class="pt-2 pl-2 text-xs font-light text-gray-400 " id="tip_group" name="tip_group">{{ $grupos ?? '' }} </span>
            </div>
        </div>
        <div class="relative w-full mb-4 sm:w-1/2">
            <div class="mx-2">
                <input type="text" id="description" name="description" wire:model='desc' value="{{ old('description') ?? ($permission<>'' ? $permission->description : '') }}"
                    class="block px-2.5 h-8 pb-1.5 pt-3 w-full text-sm font-montserrat text-gray-500 bg-transparent rounded-md border-1 border-indigo-100 appearance-none 
                    focus:outline-none focus:ring-0 focus:border-indigo-300 peer" placeholder=" " />
                <label for="description" class="absolute text-xs text-gray-500 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-5 peer-placeholder-shown:top-1/2 
                ...
                <span class="pt-2 pl-2 text-xs font-light text-gray-400 " id="tip_description1" name="tip_description1">
                    ...
                    @foreach ($ejemplos as $ej)
                        <span class="cursor-pointer" wire:click='agrega("{{$ej}}")'>
                            {{$ej}},
                        </span>
                    @endforeach
                    etc. 
